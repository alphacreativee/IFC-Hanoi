...
export function animationWorkplace() {
  gsap.registerPlugin(SplitText, ScrollTrigger);

  document.fonts.ready.then(() => {
    document.querySelectorAll("[workplace-row]").forEach((container) => {
      if (container.dataset.scriptInitialized) return;
      container.dataset.scriptInitialized = "true";

      const titleEl = container.querySelector("[workplace-title]");
      const descEl = container.querySelector("[workplace-desc]");
      const btnEl = container.querySelector("[workplace-btn]");
